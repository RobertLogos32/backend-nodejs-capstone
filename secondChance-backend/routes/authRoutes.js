...
router.put('/update', async (req, res) => {
    ...
    if (!errors.isEmpty()) {
        logger.error('Errori di validazione nella richiesta di aggiornamento', errors.array());
...
try {
    ...
    const email = req.headers.email;
        if (!email) {
            logger.error('Email non trovata nelle intestazioni della richiesta');
            return res.status(400).json({ error: "Email non trovata nelle intestazioni della richiesta" });
        }
